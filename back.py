...
def solve_captcha_if_needed(max_retry=3):
    """
    偵測並解決驗證碼，驗證是否通過
    ...
    for attempt in range(max_retry):
        ...
        if not check_captcha():
            ...
            log("[驗證碼] 驗證未通過，準備重試...")
            time.sleep(0.5)
            send_telegram(f"驗證碼重試 {attempt + 1}/{max_retry} 次")

    log(f"[驗證碼] 重試 {max_retry} 次後仍未通過")
    send_telegram(f"驗證碼重試 {max_retry} 次後仍未通過")
    return False
...
def run_fight():
    ...
    pet_full_seen = _detect_pet_full_once("開戰前")
    ...
    pet_full_seen = pet_full_seen or _detect_pet_full_once("進入戰鬥前")
    ...
    # 3. 驗證碼判斷與輸入
    log("[步驟3] 檢查驗證碼...")
    if not solve_captcha_if_needed(max_retry=3):
    ...
    # 4. 战斗中，每 5 秒检查 adx2
    log("[步驟4] 戰鬥中，等待 ADx2...")
    wait_and_click(IMG['adx2'], timeout=0, interval=5, name="ADx2")

    # 5. 等待 adx2 消失
    log("[步驟5] 等待 ADx2 消失...")
    wait_for_disappear(IMG['adx2'], timeout=0, interval=2, name="ADx2")
